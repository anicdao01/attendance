...
<body>
	...
	<div id="pageMiddle">
		<?php
			include_once('php_include/db_conx.php');
			mysql_select_db('employee');
			$query = "SELECT * FROM daily"; //You don't need a ; like you do in SQL
			$result = mysql_query($query,$db_conx);

			echo "<br /><br /><table border='1' style='width:700px;' align='center'>";
			echo "<tr><th>Date</th><th>ID</th><th>Time In</th><th>Time Out</th><th>Total Hours</th></tr>";
			while($row = mysql_fetch_array($result)){
			echo "<tr align='center'><td>" . $row['date'] ."</td><td>" . $row['id'] . "</td><td>" . $row['clock_in'] . "</td><td>" . $row['clock_out'] . "</td><td>" . $row['total_hours'] . "</td></tr>";
			}
